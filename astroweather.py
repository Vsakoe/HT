...
class AstroWeatherMod(loader.Module):
    ...
    strings = {
        ...
        "api_error": (
            "<emoji document_id=5240241223632954241>🚫</emoji> <b>Вы не указали API"
            " ключ</b>!\n<emoji document_id=5210956306952758910>👀</emoji>"
            " <code>Пожалуйста, укажите его в конфиге ниже</code>"
        ),
    ...
    async def text(self, temperature: int, veter: int, sky, hum, city, moji):
        ...
        weather = (
            f"{r_emoji} <b>Погода в {city.title()}:</b>\n\n{sity_emoji} <b>Город:</b>"
            f" <code>{city.title()}</code>\n{t_emoji} <b>Температура:</b>"
            f" <code>{temperature}°C</code>\n<emoji"
            " document_id=5192891734635322759>💦</emoji> <b>Влажность:</b>"
            f" <code>{hum}%</code>\n{v_emoji} <b>Скорость ветра:</b>"
            f" <code>{veter}м/с</code>\n{moji} <b>Небо:</b> <code>{sky}</code>"
        )
        return weather
    ...
    def __init__(self):
        self.config = loader.ModuleConfig(
            loader.ConfigValue(
                ...
                None,
                lambda: (
                    "Api key. Получить можно по туториалу https://t.me/help_code/12"
                ),
    ...
    async def get_weather(self, city_id):
        try:
            ...
            result = requests.get(
                ...
                params={"q": city_id, "units": "metric", "APPID": api_key},
            )
            ...
            if weather["sky"] == "Clouds":
                weather["sky_emoji"] = (
                    "<emoji document_id=5391322797123314747>☁️</emoji>"
                )
            if weather["sky"] == "Rain":
                weather["sky_emoji"] = (
                    "<emoji document_id=5224681716760715555>🌧️</emoji>"
                )
            if weather["sky"] == "Clear":
                weather["sky_emoji"] = (
                    "<emoji document_id=5262761021361104549>☀️</emoji>"
                )
            if weather["sky"] == "Mist":
                weather["sky_emoji"] = (
                    "<emoji document_id=5453984836668627018>🌫️</emoji>"
                )
            if weather["sky"] == "Snow":
                weather["sky_emoji"] = (
                    "<emoji document_id=5282833267551117457>🌨️</emoji>"
                )
    ...
    async def aw(self, message):
        ...
        city = city.title()
        getting = await utils.answer(
            message, self.strings("search").format(search_moji, city)
        )

        if self.config["api_key"] == None:
            await utils.answer(getting, self.strings("api_error"))
            msg = await self.client.send_message(
                message.chat.id, "<b>Открываю конфиг...</b>"
            )
