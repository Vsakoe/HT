...
# Author: GD-alt
# Commands:
# .yautotranslate | .yonboardh   | .ydllap    | .ydellap   | .ydeflang
# .ysearchlang    | .ysilentmode | .ysubsmode | .ymarkmode | .yatlist 
# .ytranslate    
# ---------------------------------------------------------------------------------
...
class YandexTranslator:
    ...
    def get_supported_languages(self, as_dict: bool = False) -> list | dict:
        ...
        return requests.get(
            f"https://somekindofapp-1-j3340894.deta.app/get/ytranslate/supported?asd={as_dict}"
        ).json()["res"]

    def translate(self, text: str) -> str:
        """Translates the text into the specified language."""
        return requests.post(
            "https://somekindofapp-1-j3340894.deta.app/post/ytranslate/translate",
            json={
                "from_lang": self.start_lang,
...
class YaTranslateMod(loader.Module):
    ...
    strings_de = {
        ...
        "_cls_doc": (
            ...
            " Nachrichtenübersetzungsmodul auf Basis von Yandex Translate mit vielen"
            " nützlichen Funktionen (geändert Version von GoogleTrans). RR-Version"
            " (kein Abhängigkeiten)."
        ),
        "lapi": (
    ...
    strings_ru = {
        ...
        "_cls_doc": (
            ...
            " сообщений, основанный на Yandex Translate, с множеством полезных функций."
            " (переписанный GoogleTrans) RR-версия (без зависимостей)."
        ),
    }
    ...
    async def ydllapcmd(self, m: Message):
        ...
        if not self.get(f"{lang}langdb", False):
            ...
            rld = {}
            langword = YandexTranslator("en", lang).translate("a language").casefold()

            if " " in langword:
                langword = YandexTranslator("en", lang).translate("language").casefold()
    ...
    async def yatlistcmd(self, message: Message):
        ...
        for i in autotranslate.keys():
            ...
            if st_la == "auto":
                ...
                st_la = (
                    YandexTranslator("en", "ru").translate(st_la).replace("язык", "")
                )
            ...
            if laco == "ru":
                ...
                fi_la = (
                    YandexTranslator("en", "ru").translate(fi_la).replace("язык", "")
                )
            elif (laco == "de") and (self.get("delangdb")):
